#!/bin/bash
#SBATCH --job-name=edit_names
#SBATCH --output=edit_names_%A_%a.out
#SBATCH --error=edit_names_%A_%a.err
#SBATCH --array=0-18%1
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=2
#SBATCH --cpus-per-task=8
#SBATCH --mem=44G
#SBATCH --time=48:00:00

# Load Python and activate the environment

. /home/YOURNAME/miniforge3/etc/profile.d/conda.sh

. /home/YOURNAME/miniforge3/etc/profile.d/mamba.sh

mamba activate chordo2

input_files=($(ls *_1*.fastq | sed -r 's/_1[.]fastq//' | uniq))

# Use SLURM_ARRAY_TASK_ID to process only one file pair at a time
f=${input_files[$SLURM_ARRAY_TASK_ID]}


# Edit name for making Trinity work

sed -E "s/^((@|\+)SRR[^.]+\.[^.]+)\.(1|2)/\1/" "$f"_1.fastq > "$f"_edit_1.fastq

sed -E "s/^((@|\+)SRR[^.]+\.[^.]+)\.(1|2)/\1/" "$f"_2.fastq > "$f"_edit_2.fastq